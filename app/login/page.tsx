...
const Page = () => {
  ...
  const handleLogin = async(e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true)
    await axios.post(`/api/auth/login`, loginDetails)
    .then((res) => {
      if(res.data.token){
    ...
    <>
      <div className="flex justify-center h-screen items-center p-4 ">
        <div className="grid grid-row-2 lg:grid-cols-2 border p-4 rounded-xl border-slate-600 shadow-lg shadow-red-600 w-[90%] lg:w-[50rem] ">
          <form onSubmit={handleLogin} className="flex flex-col lg:border-r p-4  border-slate-600  ">
          ...
          <div className="mb-8 flex flex-col ">
            ...
            <div className="mt-4">
              <button type="submit" className={`border 
                p-2 rounded-lg  border-red-600 text-red-600 hover:bg-red-600 hover:text-white  font-semibold ${loading? 'opacity-50 cursor-not-allowed ' : ''}`} >
                {loading? 'Logging in...' : 'LogIn'}
              </button>
            </div>
          </div>
          <p className={`text-sm flex gap-2 justify-center`}> Do not have an account? <Link href={'/signup'} className={`text-red-600 ${loading? 'opacity-50 cursor-not-allowed ' : ''}`}> SignUp </Link> </p>
          </form>
